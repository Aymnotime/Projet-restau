...
import {
  BOBIGNY_RESTAURANT as RESTAURANT,
  BOBIGNY_ORDER_URL,
  GOOGLE_BOBIGNY_REVIEWS_URL,
  SITE_URL,
} from "../data/site";
...
const MARQUEE = [
  "Fait maison", "Bobigny", "Livraison & retrait", "Saveurs du monde",
    ...
    <section ref={ref} className="relative flex h-svh min-h-[420px] sm:min-h-[500px] md:min-h-[560px] lg:min-h-[620px] flex-col overflow-hidden">
      <motion.div className="absolute inset-0" style={reduce ? undefined : { y }}>
        <div className="animate-kenburns motion-reduce:animate-none absolute inset-0">
          <img
            src={IMAGES.hero}
            alt="Sandwich généreux du Monde du Goût à Bobigny, fromage fondant, lumières urbaines en arrière-plan"
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 flex-col justify-end px-3 sm:px-4 lg:px-8 pb-12 sm:pb-16 lg:pb-20">
        ...
        <Reveal delay={0.85}>
          <p className="mt-2.5 max-w-xl text-xs sm:text-sm leading-relaxed text-sand sm:mt-3 lg:text-base">
            Sandwichs signatures & cuisine du monde à Bobigny (93).
          </p>
        </Reveal>
        <Reveal delay={1}>
          <div className="mt-3.5 flex flex-wrap items-center gap-2.5 sm:gap-3 sm:mt-5">
            <a
              href={BOBIGNY_ORDER_URL || RESTAURANT.phoneHref}
              target={BOBIGNY_ORDER_URL ? "_blank" : undefined}
              rel={BOBIGNY_ORDER_URL ? "noopener noreferrer" : undefined}
    ...
    <section className="border-b border-graphite bg-soot" aria-label="Informations pratiques">
      <div className="mx-auto grid max-w-7xl divide-y divide-graphite px-4 sm:px-6 md:grid-cols-4 md:divide-x md:divide-y-0 lg:px-8">
        <a
          ...
          aria-label={`Voir les avis Google du Monde du Goût Bobigny, note ${RESTAURANT.googleRating} sur 5 et ${RESTAURANT.googleReviewCount} avis`}
        >
        ...
        <div className="flex items-center gap-3 py-4 md:px-6 md:py-5 md:first:pl-0">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted">Bobigny (93)</p>
          </div>
        </div>
...
function LocationCard() {
  return (
    <section className="mx-auto max-w-7xl px-4 py-16 sm:py-20 lg:py-24">
      <SectionMark n="01" label="Notre restaurant" right="Bobigny — 93" />
        ...
        <Reveal delay={0.1}>
          <div className="grid gap-4 sm:gap-6 sm:grid-cols-2">
            <div className="border border-graphite bg-soot p-5 sm:p-6">
              ...
              <p className="font-display text-xl sm:text-2xl text-cream">11h00 — 2h00</p>
            </div>
            <div className="border border-graphite bg-soot p-5 sm:p-6">
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-2">
        <Reveal>
          <article className="h-full border border-ember/60 bg-soot p-6">
            <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.3em] text-ember">Établissement principal</p>
        ...
        <Reveal delay={0.1}>
          <article className="h-full border border-graphite bg-soot p-6">
            <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.3em] text-ember">Notre autre restaurant</p>
            <h3 className="mt-3 font-display text-2xl sm:text-3xl tracking-wide">BOBIGNY</h3>
            <address className="mt-2 text-xs sm:text-sm not-italic leading-relaxed text-sand">
...
function Signatures() {
  return (
    <section className="mx-auto max-w-7xl px-4 py-16 sm:py-20 lg:py-24">
      <SectionMark n="02" label="Les signatures" right="Bobigny" />
      <div className="mt-8 flex flex-wrap items-end justify-between gap-4 sm:gap-6">
        ...
        <Reveal delay={0.2}>
          <p className="max-w-xs text-xs sm:text-sm leading-relaxed text-muted">Nos recettes phares à Bobigny.</p>
        </Reveal>
      </div>
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-14 sm:py-16 lg:py-20 text-center sm:px-6 lg:px-8">
        ...
        <Reveal delay={0.3}>
          <p className="mx-auto mt-3 sm:mt-4 max-w-md text-xs leading-relaxed text-sand">
            Commandez en ligne ou par téléphone. Livraison à domicile dans le secteur de Bobigny.
          </p>
        </Reveal>
        <Reveal delay={0.4}>
...
export default function BobignyPage() {
  useEffect(() => {
    document.title = "Le Monde du Goût – Restaurant à Bobigny | Sandwichs & Cuisine du Monde";
    const meta = document.querySelector('meta[name="description"]');
    if (meta) {
      meta.setAttribute("content", "Restaurant Le Monde du Goût à Bobigny (93000). Sandwichs signatures inspirés des cuisines du monde, faits maison. Livraison et retrait sur place. Ouvert 7j/7 de 11h00 à 2h00.");
    }
  }, []);

  // Structured data for Bobigny restaurant
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "Restaurant",
    "name": RESTAURANT.name,
    "image": `${SITE_URL}${IMAGES.hero}`,
    "url": `${SITE_URL}/bobigny`,
